...
router.post('/add-user', async function (req, res, next) {
    ...
    if (req.body.begin_date >= req.body.end_date)
        res.render('rooster/add-user', { title: "Gebruiker inroosteren", user_list: await userDBHandler.getAllUsers(), error: "Begintijd mag niet later zijn dan eindtijd", success: false });
    else
