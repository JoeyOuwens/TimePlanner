...
exports.seed = function(knex) {
  // Deletes ALL existing entries
    return knex('substitute').del()
    .then(function () {
      // Inserts seed entries
        return knex('substitute').insert([
            { id: 0, requesting_user: 1, replaced_by_user: 1, creation_datetime: new Date().setMonth(7),timetable_item: 2,  status: 'AWAITING_REPLACEMENT', comment: '' },
            { id: 1, requesting_user: 2, replaced_by_user: 3, creation_datetime: new Date().setMonth(5),timetable_item: 1,  status: 'AWAITING_APPROVAL', comment: '' },
